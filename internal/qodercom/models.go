...
func (c *Client) fetchModels(a *auth.Auth) ([]ModelEntry, error) {
	...
	rawURL := c.ModelsBase + EpModels // COM 双域名分离：模型表 api2，推理 api1（qoder2api Global 表同构）
...
func (c *Client) FetchModels(a *auth.Auth) ([]provider.ModelInfo, error) {
	...
	if err != nil {
		if cached := c.cachedModels(); len(cached) > 0 {
			log.Printf("qodercom models fetch failed (%v), using last-success cache %d models", err, len(cached))
...
func (c *Client) FetchModelPricing(a *auth.Auth) ([]provider.ModelPricing, error) {
	...
	if err != nil {
		if cached := c.cachedModels(); len(cached) > 0 {
			log.Printf("qodercom pricing fetch failed (%v), using last-success cache", err)
